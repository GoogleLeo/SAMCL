# -*- coding: utf-8 -*-
"""CPR2.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1wIlLasdnwcnksBOw3VFk3W2jdk83JHGJ
"""

import torch
import torch.nn as nn
import numpy as np
import torch.optim as opti
from torch.utils.data import DataLoader
from copy import deepcopy
from torch.nn import functional as F
from tqdm.notebook import tqdm
from torchvision import transforms
from torchvision.models import resnet18
from pyhessian import hessian
from utils import *
from nngeometry.metrics import FIM
from nngeometry.object import PMatKFAC, PMatDiag, PVector
from continuum.datasets import CIFAR10
from continuum.datasets import MNIST
from continuum import ClassIncremental
from continuum import InstanceIncremental
from continuum.datasets import PyTorchDataset
import torchvision
from nngeometry.object import PMatDense
import torchvision.models as models
import torch.nn.init as init

if torch.cuda.is_available():
    device = torch.device("cuda")
    print("GPU is available")
else:
    device = torch.device("cpu")
    print("GPU is not available")

import random
def set_seed(seed):
    torch.manual_seed(seed)
    np.random.seed(seed)
    random.seed(seed)
    if torch.cuda.is_available():
        torch.cuda.manual_seed(seed)
        torch.cuda.manual_seed_all(seed)
        torch.backends.cudnn.deterministic = True
        torch.backends.cudnn.benchmark = False

set_seed(1)

class CPR(nn.Module):
    def __init__(self):
        super(CPR, self).__init__()

    def forward(self, x):
        b = F.softmax(x, dim=1) * F.log_softmax(x, dim=1)
        b = -1.0 * b.sum(dim=1)

        return b.mean()

class Net(nn.Module):
    def __init__(self):
        super(Net, self).__init__()
        self.conv1 = nn.Conv2d(3, 32, 3, padding=1)
        self.pool = nn.MaxPool2d(2, 2)
        self.conv2 = nn.Conv2d(32, 64, 3, padding=1)
        self.fc1 = nn.Linear(64 * 8 * 8, 128)
        self.fc2 = nn.Linear(128, 10)

    def forward(self, x):
        x = self.pool(nn.functional.relu(self.conv1(x)))
        x = self.pool(nn.functional.relu(self.conv2(x)))
        x = x.view(-1, 64 * 8 * 8)
        x = nn.functional.relu(self.fc1(x))
        x = self.fc2(x)
        return x

# Train set
Trainset = CIFAR10(data_path="./data", download=True, train=True)
scenario1 = ClassIncremental(Trainset, increment=2)

#Test set
Testset = CIFAR10(data_path="./data", download=True, train=False)
scenario2 = ClassIncremental(Testset, increment=2)

def CPR_EWC(model, BS, optimizer, criterion, scenario1, scenario2, Initial_WD, Lambda, beta, epoch):
  WD = Initial_WD
  FisherList = []
  Total_BS = 10000
  A = 0
  cpr = CPR()
  G = []
  v_List = []
  G2 = []
  Temp_Epoch = 0
  Acc_L = []
  Acc_Train = []
  Bests = []

  Norm_List = []

  for task_id, taskset in enumerate(scenario1):
      model.train()
      Norm_List = []

      if(task_id == 4):
           epoch = 50
      print(f"Task: {task_id}, Number of Samples: {len(taskset)}")
      trainLoader = DataLoader(deepcopy(taskset), batch_size=BS, shuffle=True, num_workers=2)
      for E in range(epoch):
            G = []
            G2 = []
            for inputs, labels, t in trainLoader:
                 #Zero the parameter gradients
                 optimizer.zero_grad()

                 #Forward pass
                 outputs = model(inputs.to(device))

                 #Calculate the loss
                 Loss = criterion(outputs, labels.to(device))
                 Loss = Loss - (beta * cpr(outputs))



                 v = PVector.from_model(model)
                 if(task_id == 0):
                     #Add Regularization
                     Loss = Loss + WD*(1/2)*v.dot(v)


                 else:
                     #Recover the total loss
                     for i in range(len(FisherList)):
                           Loss = Loss + Lambda * ( (1/2) * MatrixOP(FisherList[i], v, v_Last))
                     Loss = Loss +  ((1/2) * WD * DotOP(v, v_Last))



                 Loss = Loss.to(device)


                 #Calculate Grad
                 G.append(Grad_Monitor(model, inputs.to(device), labels.to(device), criterion))
                 if(task_id != 0):
                      G2.append(Grad_old(FisherList, model, v_Last, Lambda))


                 #Train
                 Loss.backward()
                 optimizer.step()




            Norm_New = np.mean(G)
            Norm_old = np.mean(G2)
            if(task_id !=0):
                #Dist = Monitor_Dist(model, v_Last)
                print(f"Task: {task_id}, epoch: {E}, Norm_New: {Norm_New}, Norm_old: {Norm_old}")
                Norm_List.append(Norm_New + Norm_old)

            else:
                print(f"Task: {task_id}, epoch: {E}, Norm_New: {Norm_New}")
                Norm_List.append(Norm_New)



            if(task_id == 4):
                  Temp =  Acc(scenario2, model, task_id)
                  if(Temp > A):
                        A = Temp
                        Temp_Epoch = E+1
                        Temp_v = PVector.from_model(model).clone().detach()
                        print(f"A : {A}")




      #Test
      if(task_id == 4):
            Temp_v.copy_to_model(model)


      Acc_L.append(Acc(scenario2, model, task_id))
      Acc_Train.append(Acc(scenario1, model, task_id))
      Bests = Bests_Update(scenario1, model, task_id, Bests)


      #store old information
      TempLoader = DataLoader(deepcopy(taskset), batch_size=len(taskset), shuffle=True, num_workers=2)
      F_kfac = FIM(model=model,
                          loader = TempLoader,
                          representation = PMatDiag,
                          n_output=10,
                          variant='classif_logits',
                          device = 'cuda')
      FisherList.append(F_kfac)
      v_Last = PVector.from_model(model).clone().detach()
      v_List.append(v_Last)
      Total_BS = Total_BS + len(taskset)

  if(task_id == 4):
      AAA_Test = (1/len(Acc_L)) * np.sum(Acc_L)
      print(f"AAA_Test: {AAA_Test}")
      AAA_Train = (1/len(Acc_Train)) * np.sum(Acc_Train)
      print(f"AAA_Train: {AAA_Train}")
      FM(scenario1, model, Bests)

model = Net()
model = model.to(device)
criterion = nn.CrossEntropyLoss()
optimizer = optim.SGD(model.parameters(), lr=0.000025)
CPR_EWC(model, BS = 16, optimizer, criterion, scenario1, scenario2, Initial_WD=0, Lambda=120, beta=0.9, epoch=50)